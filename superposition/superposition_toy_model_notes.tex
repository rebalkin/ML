...
\title{Phase Transitions in Sparse Feature Representations}
...
\begin{abstract}
These notes derive the analytical results underlying the toy superposition model notebooks. We follow the setup of Elhage et al.\ (2022) and study a minimal one-hidden-unit autoencoder ($n=2$ features, $m=1$ latent dimension) trained on sparse, independently distributed inputs. Section~1 treats the linear model and shows that superposition cannot occur: the only minima of the expected loss are the single-feature solutions $\theta=0$ and $\theta=\pi/2$, and the non-trivial critical point is always a saddle. Section~2 introduces a ReLU nonlinearity and derives a three-phase structure (including a genuine superposition minimum at $\theta=3\pi/4$) via exact closed-form expressions for the expected loss.
\end{abstract}
...
\subsection{Both features active} ($x_1,x_2\sim\mathrm{Unif}[0,1]$, probability $p_1 p_2$). Now both inputs enter the reconstruction, coupling $x_1$ and $x_2$. Integrating over both requires the double auxiliary integrals
\begin{align}
...
The two models studied here bracket the role of the nonlinearity. In the linear case the expected loss is a polynomial in $\theta$ whose only minima are at $\theta=0$ and $\theta=\pi/2$; the non-trivial stationary point is always a saddle, so superposition is impossible. The ReLU breaks this symmetry: the reconstruction of a positively-activated feature and a negatively-activated one are decoupled by the nonlinearity, opening up the antipodal configuration $\theta=3\pi/4$ as a genuine minimum. The resulting phase boundary depends on $I_i$ and $p_i$ only through their product $q_i = I_i p_i$ at leading order in sparsity, collapsing the four-parameter family to a two-dimensional phase diagram. Corrections at finite $p_i$ are encoded in the $D$ term and are explored numerically in the companion notebooks.

\begin{thebibliography}{9}
\bibitem{elhage2022} Elhage et al., \textit{Toy Models of Superposition}, Transformer Circuits Thread (2022). \texttt{https://transformer-circuits.pub/2022/toy\_model/index.html}
\end{thebibliography}
\end{document}