...
The expected loss is a simple closed-form polynomial in $(\cos\theta, \sin\theta)$. Minimising over the biases $b^*(\theta)$ and then over $\theta$ reveals:
...
Minimising over $(\theta, b_1, b_2)$ reveals a **three-phase structure** governed by the effective importances $q_i = I_i p_i$:
...
| 3 | Phase diagram from 4-D grid scan; phase boundaries verified |
